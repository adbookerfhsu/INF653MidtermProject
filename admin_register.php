<?php
    ...
    if (empty($username)) {
        ...
        if($statement->fetchColumn()) {
            $error_username = "The username you enters is already taken.";
        }
    }
    // check password
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" >
            ...
            <input type="text" name="username"><span class="error_message"><?php if(!empty($error_username)) echo $error_username;?></span>
            <br>

            <label for="password">Password:</label>
            <input type="password" class="text" name="password" title="Must contain at least one number, one uppercase letter, one lowercase letter and be 8 or more characters."><span class="error_message"><?php if(!empty($error_password)) echo $error_password; ?></span>

            <label for="confirmpassword">Confirm Password:</label>
            <input type="confirmpassword" name="confirmpassword" title="Must contain at least one number, one uppercase letter, one lowercase letter and be 8 or more characters."><span class="error_message"><?php if (!empty($error_confirmpassword)) echo $error_confirmpassword; ?></spn>
